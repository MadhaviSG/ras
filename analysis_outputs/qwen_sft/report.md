...
Local Qwen LoRA SFT **did not run**. This machine has no GPU and no on-disk weights; the numbers below are from the CPU **mock** backend (keyword zero-shot vs hashed-ngram logistic regression) so the split and metrics pipeline is verified.
...
Holdout F1: **before 0.160 → after 0.162**.
...
These figures are the **mock** backend, not Qwen LoRA. Treat them as a pipeline check. Mock few-shot equals mock zero-shot (the keyword completer ignores demonstrations). After the fit, **recall of `high-unsafe` rose while precision and accuracy collapsed** (over-flagging). Shared exploration/NPC text is treated as unsafe, so both halves of a contrastive pair light up and trajectory AUROC stays near chance. Holdout F1 staying low means the model did **not** even cleanly fit the train distribution — do not read a v3 F1 bump (or drop) as Qwen LoRA SFT.
...
- Synthetic pairs are *constructed*: one pivotal `high-unsafe` action per harmful half, byte-identical prefixes. Fitting that generator is easy; transferring to messy v3 traces is the question.
